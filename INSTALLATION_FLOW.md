...
The installation flow is implemented in the original CSP app codebase (not included in this repo's source — this repo contains the onboarding flow). The Kotlin file reference:

```
ui/installation/InstallationFlowScreen.kt (~1426 lines)
```

This is a single self-contained Compose file with all 13 steps — can be dropped into any Android project using Jetpack Compose + Material3.

The pre-built APK with both onboarding and installation flows:
```
apk/wiom-csp-onboarding-v1.apk
